...
import com.beacon.framework.common.util.collection.SetUtils;
...
public class ProjectReactor {
    ...
    private static final String GROUP_ID = "com.beacon.boot";
    ...
    public static void main(String[] args) {
        ...
        String artifactIdNew = "hello";
        //包的路径
        String packageNameNew = "com.hello";
        //项目名称，前端项目中的项目名称
        ...
        if (StrUtil.containsAny(newProjectBaseDir, PACKAGE_NAME, ARTIFACT_ID, StrUtil.upperFirst(ARTIFACT_ID))) {
            log.error("[main][新项目目录检测 ({}) 存在冲突名称「{}」或者「{}」，请更改新的目录！程序退出]", newProjectBaseDir, PACKAGE_NAME, ARTIFACT_ID);
            return;
        }
        log.info("[main][完成新项目目录检测，新项目路径地址 ({})]", newProjectBaseDir);
        // 获得需要复制的文件
        ...
        files.forEach(file -> {
            ...
            if (WHITE_FILE_TYPES.contains(fileType)) {
                copyFile(file, projectBaseDir, newProjectBaseDir, packageNameNew, artifactIdNew);
                return;
            }
            // 如果非白名单的文件类型，重写内容，在生成文件
            String content = replaceFileContent(file, groupIdNew, artifactIdNew, packageNameNew, titleNew);
            writeFile(file, content, projectBaseDir, newProjectBaseDir, packageNameNew, artifactIdNew);
        });
    ...
    private static String replaceFileContent(File file, String groupIdNew, String artifactIdNew, String packageNameNew, String titleNew) {
        ...
        return content.replaceAll(GROUP_ID, groupIdNew)
                ...
                // ARTIFACT_ID 相比于group_id 和 package_name 短 若放在前面处理会导致误伤
