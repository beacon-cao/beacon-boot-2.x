package com.beacon.business.system.controller.admin.user;
...
import com.beacon.business.system.convert.user.UserConvert;
...
public class UserController {
    ...
    @PreAuthorize("@security.hasPermission('system:user:update-password')")
    ...
    @Parameter(name = "id", description = "编号", required = true, example = "1024")
    @PreAuthorize("@security.hasPermission('system:user:query')")
    ...
    public void importTemplate(HttpServletResponse response) throws IOException {
        ...
        List<UserImportExcelVO> list = Arrays.asList(
                UserImportExcelVO.builder().username("test1").deptId(1L).email("test1@xx.cn").mobile("18888888888").nickname("测试1").status(CommonStatusEnum.ENABLE.getStatus()).sex(SexEnum.MALE.getSex()).build(),
                UserImportExcelVO.builder().username("test2").deptId(2L).email("test2@xx.cn").mobile("19999999999").nickname("测试2").status(CommonStatusEnum.DISABLE.getStatus()).sex(SexEnum.FEMALE.getSex()).build()
        );
